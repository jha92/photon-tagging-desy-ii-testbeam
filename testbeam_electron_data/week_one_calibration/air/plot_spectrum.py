...
ax.legend([name], title=('%1.1f GeV'%float(data_name[:3].replace("_", "."))), fontsize='small')
# options
plt.yscale('log')
plt.ylim(1., 1e4)
...
plt.text(0.05, 0.95, fit_info, fontsize=8, transform = ax.transAxes,
...
with open('fitparameter.txt','a') as outfile:
	outfile.write('%.1f,%.4f,%.4f\n'%(float(data_name[:3].replace("_", ".")),para[0],abs(para[1])))
...
### plot distribution with energy scale
#fig, ax = plt.subplots(figsize=(4, 3), dpi=300)
#fig.subplots_adjust(left=0.17, right=0.95, top=0.95, bottom=0.17)
#binsE = 0.083+1.2949*bins
#binningE = binsE[1] - binsE[0]
#plt.bar(binsE, counts, width=binningE, color=color_name)
#plt.text(0.75, 0.95, data_info, fontsize=8, transform = ax.transAxes,
...
#plt.ylabel(r'counts [#]')
#plt.xlim(0.,4.5)
#save_name = script_name[:-3] + '_' + data_name[:-4] + "_energyScale.png"
#plt.savefig(save_name)

### plot normalized distribution with energy scale 
#fig, ax = plt.subplots(figsize=(4, 3), dpi=300)
#fig.subplots_adjust(left=0.17, right=0.95, top=0.95, bottom=0.17)
#plt.bar(binsE, counts_norm, width=binningE, color=color_name)
#plt.text(0.75, 0.95, data_info, fontsize=8, transform = ax.transAxes,
#         verticalalignment='top', horizontalalignment='left', bbox=props)
#plt.yscale('log')
#plt.ylim(1e-5, 3e-1)
...
#plt.ylabel(r'normalized counts')
#plt.xlim(0.,4.5)
#save_name = script_name[:-3] + '_' + data_name[:-4] + "_energyScaleNorm.png"
#plt.savefig(save_name)
